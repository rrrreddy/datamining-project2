...
model_perceptron = Perceptron(max_iter=20,eta0=1,random_state=3)
model_perceptron.fit(X_train, y_train)
y_valid_pred = model_perceptron.predict(X_valid)
y_test_pred = model_perceptron.predict(df_test)
df_test_pred = pd.DataFrame(y_test_pred)
df_test_pred = np.where((df_test_pred==0),'<=50K','>50K')
pd.DataFrame(df_test_pred).to_csv('pred_perc_3.csv')
perc_report = classification_report(y_valid,y_valid_pred)
print("perceptron model classification_report" ,'\n',perc_report)
