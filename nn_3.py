...
model_nn = MLPClassifier(hidden_layer_sizes=(2,2),learning_rate_init=0.001,random_state=3)
model_nn.fit(X_train, y_train)
y_valid_pred = model_nn.predict(X_valid)
y_test_pred = model_nn.predict(df_test)
df_test_pred = pd.DataFrame(y_test_pred)
df_test_pred = np.where((df_test_pred==0),'<=50K','>50K')
pd.DataFrame(df_test_pred).to_csv('pred_nn_3.csv', header=None, index=False)
perc_report = classification_report(y_valid,y_valid_pred)
print("neural network model classification_report" ,'\n',perc_report)
